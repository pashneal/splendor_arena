...
pub async fn push_game_update(outgoing_stream: Outgoing, arena: GlobalArena) {
    let mut outgoing_stream = outgoing_stream.write().await;
    let arena = arena.read().await;
    let game_update = get_game_update(&arena)
        .await
        .expect("Failed to get game update");
...
pub async fn push_initial_game(
    ...
    while let Some(msg) = incoming_stream.next().await {
        ...
        if msg.is_err() {
            error!(
                "Failed to deserialize message from global server: {}",
                msg.err().unwrap()
            );
            handle_failure();
            return None;
...
pub async fn push_authentication(
    ...
    info!("Contacting stourney.com...");
    {
        ...
        outgoing_stream
            .send(message)
            .await
            .expect("Failed to send authentication request");
    }
    ...
    while let Some(msg) = incoming_stream.next().await {
        ...
        let msg = msg.expect("Failed to receive message from global server");
        let msg = msg.to_string();
        let msg: GlobalServerResponse =
            serde_json::from_str(&msg).expect("Failed to deserialize message from global server");
        debug!("Received message from global server: {:?}", msg);
...
pub async fn start(arena: GlobalArena) -> Result<(Outgoing, Incoming), String> {
    ...
    let auth =
        push_authentication(outgoing_stream.clone(), &mut incoming_stream, arena.clone()).await;
    if !auth {
    ...
    println!(
        "The game is running in the background and all logs will go to ./{}",
